...
- **REQ-FAC-004 (IO & Bus Debug Real-Time Telemetry Stream)**: Provide a dedicated, responsive IO & Bus Debug dashboard tab streaming discrete TCA9555 IO expander states (12 vehicle switches/telltales), raw 16-bit ADS1115 ADC voltages without SMA filtering scaled to the 0–4096 mV hardware caliber (`max="4096"`), MCPWM capture frequencies (Coolant, RPM, Speed), and CAN node status/firmware versions (LDB and RDB) over WebSockets (`/ws`) at a configurable broadcast rate (default 5 Hz via `CONFIG_FACTORY_WS_REFRESH_RATE_HZ`).
- **REQ-FAC-005 (Real-Time OTA Progress Streaming)**: Broadcast real-time firmware flashing progress (percentage, transferred bytes, total size, transfer speed, and status messages) over WebSockets (`/ws`) to dynamically update UI progress indicators during local multipart HTTP uploads and CAN bus segmented transfers.
...
| **REQ-FAC-004** | Factory Apps | [`factory apps/ITF factory app/main/main.cpp`](../factory%20apps/ITF%20factory%20app/main/main.cpp), [`factory apps/ITF factory app/main/web/index.html`](../factory%20apps/ITF%20factory%20app/main/web/index.html) | WebSocket telemetry stream inspection at 5 Hz & Playwright UI verification |
| **REQ-FAC-005** | Factory Apps | [`factory apps/ITF factory app/main/main.cpp`](../factory%20apps/ITF%20factory%20app/main/main.cpp), [`factory apps/ITF factory app/main/web/index.html`](../factory%20apps/ITF%20factory%20app/main/web/index.html) | WebSocket OTA progress frame stream audit during `/upload` and `/flash` |
