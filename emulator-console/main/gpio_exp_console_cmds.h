...
static int setExpIO(int argc, char **argv) {
	...
	if (expanders[setExpIO_args.exp_id->ival[0]]->digitalWrite(setExpIO_args.gpio_id->ival[0],
															   setExpIO_args.level->ival[0]) == false)
		return 1;
	// Feedback
	printf("GPIO %u on expander %u set to %s \n", setExpIO_args.gpio_id->ival[0], setExpIO_args.exp_id->ival[0],
		   setExpIO_args.level->ival[0] == 1 ? "HIGH" : "LOW");
	return 0;
}
...
static void register_setExpIO(void) {
	...
	const esp_console_cmd_t cmd = {.command = "setExpIO",
...
static void register_printExpStatus(void) {
	...
	const esp_console_cmd_t cmd = {.command = "printExpStatus",
...
static int set_ignition(int argc, char **argv) {
	...
	if (setActHL_args.level->count > 0) {
		...
		printf("%s set LL INPUT to %s, mask %lx \n", nickname, (setActHL_args.level->ival[0]) ? "HIGH" : "LOW",
			   (uint32_t)(expanders[0]->multiDigitalRead(0xFFFF)));
...
static int setAllExpIO(int argc, char **argv) {
	...
	if (expanders[setAllExpIO_args.exp_id->ival[0]]->multiDigitalWrite(0xFFFF, setAllExpIO_args.level->ival[0]) ==
		false)
		return 1;
	// Feedback
	printf("GPIOs on expander %u set to %s \n", setAllExpIO_args.exp_id->ival[0],
		   setAllExpIO_args.level->ival[0] == 1 ? "HIGH" : "LOW");
...
static void register_setAllExpIO(void) {
	...
	const esp_console_cmd_t cmd = {.command = "setAllExpIO",
...
static void register_setxxxResOut(void) {
	...
	const esp_console_cmd_t cmd_low = {.command = "setLowResOut",
									   .help =
										   "Set the resistor emulator to a low output caliber with a certain divider",
									   .hint = NULL,
									   .func = &setLowResOut,
									   .argtable = &setxxxResOut_args};

	const esp_console_cmd_t cmd_high = {.command = "setHighResOut",
										.help =
											"Set the resistor emulator to a high output caliber with a certain divider",
										.hint = NULL,
										.func = &setHighResOut,
										.argtable = &setxxxResOut_args};
...
static float fuelResValues[19] = {30.2,	 39.6,	50.0,  59.5,  70.9,	 79.3,	90.0,  100.9, 112.3, 118.9,
...
static void register_getExpMask(void) {
	...
	const esp_console_cmd_t cmd = {.command = "getExpMask",
...
static void register_setExpMask(void) {
	...
	const esp_console_cmd_t cmd = {.command = "setExpMask",
