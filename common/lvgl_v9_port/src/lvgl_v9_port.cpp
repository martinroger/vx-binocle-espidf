...
static lv_display_t *display_init(LCD *lcd)
{
    ...
    lv_display_set_buffers(disp,lvgl_buf[0],lvgl_buf[1],buffer_size*lv_color_format_get_size(lv_display_get_color_format(disp)),LV_DISPLAY_RENDER_MODE_DIRECT);
...
bool lvgl_port_init(LCD *lcd, Touch *tp)
{
    ...
    if (bus_type != ESP_PANEL_BUS_TYPE_RGB) {
        ...
        lcd->attachDrawBitmapFinishCallback(onDrawBitmapFinishCallback, disp);
    }
